...
INITAIL_BUFFER_DIS = 6  # 初始态匹配时，缓存池大小（m） >= BUFFER_DIS!
    ...
    def mag_position_thread(self, in_data_queue, out_data_queue, coordinate_offset, entrance_list) -> None:
        ...
        while True:
            ...
            if self.state == MagPositionState.STABLE_RUNNING:
                # 稳定运行态
                # TODO 基于 inital_transfer 按照上面的状态写法，写后续的匹配:滑动窗口匹配
                ...
                while self.state == MagPositionState.STABLE_RUNNING:
                    ...
                    window_buffer.append(cur_data)
                    temp_dis += math.hypot(cur_data[1][0] - last_data[1][0], cur_data[1][1] - last_data[1][1])

                    if temp_dis >= slide_distance:
                        temp_dis = 0
                        # 达到滑窗距离，调用一次匹配算法，并将结果只应用到新加的那段坐标
